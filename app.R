...
if(interactive()){
	ui = fluidPage(
		titlePanel("No sparge brewhouse efficiency calculator"),
		sidebarLayout(
			sidebarPanel(
			...
			mainPanel(
				plotOutput("ogPlot", height="300px", width="500px"),
				plotOutput("effPlot", height="300px", width="500px")
	...
	server = function(input, output){
		brewcalc = function(){
			...
			efficiency =  (((OG - 1) * input$batchsize) / ((input$ppg - 1) * grain)) * 100
